...
G0_plocal_stat = function(t,j,sigma,burn){ # burn should be j-500
  if(j <= burn){
    return(0)
  }else{
    out = 0.5*G0_plocal_stat(t,j-1,sigma,burn=burn)+rnorm(n=1,sd=1 )
    return(out)
  }
}
G1_plocal_stat = function(t,j,sigma,burn){ # burn should be j-500
  if(j <= burn){
    return(0)
  }else{
    out = -0.5*G1_plocal_stat(t,j-1,Sigma_e,burn=burn)+rnorm(n=1,sd=1 )
    return(out)
  }
}
#G_plocal_stat(1,n,Sigma_e,burn = n-500)
PLS_process = function(n,Burn=500,sigma){
  outputs = rep(NA,n)
  for (i in 1:floor(n/3)) outputs[i] = G0_plocal_stat(i/n,i,sigma,burn = i-Burn)
  for (i in (floor(n/3)+1):n) outputs[i] = G1_plocal_stat(i/n,i,sigma,burn = i-Burn)
  return(outputs)
}
...
simulation_estiamtion_reg = function(hr,hd,sigma){
  # simulation+estimation #
  LS1 = PLS_process(n = n,sigma = sigma)
  ...
  # long-run variance estimation
  m_star = MV_choice(residuals = residuals_m,candidates = 1:25)
...
hd.opt = c()
for (i in hrs) {
  hd.opt = c(hd.opt, min(i^2*n^0.01,i/n^0.005 )) # ensure hd is between (hr^2,hr)
}
hds = hd.opt
...
for (p in 1:nrow(bandwidth_choices)) {
  ...
  # output coverage
  # m12
  coverage.m12[p,2:5] = apply(matrix(unlist(lapply(cover, "[[",1)),byrow = T,ncol = 4)[,1:4],FUN = mean,MARGIN = 2)
  
  print(coverage.m12[p,])
}

name = paste0(args,'batch',B,'boot',simulations,'PLS_simul_SCB_reg_est','n',n,'.csv')
